...
static PyObject *grid_subsampling_compute(PyObject *self, PyObject *args, PyObject *keywds)
{
	...
	PyArrayObject *points_array_ptr = reinterpret_cast<PyArrayObject *>(points_array);
	PyArrayObject *features_array_ptr = use_feature ? reinterpret_cast<PyArrayObject *>(features_array) : nullptr;
	PyArrayObject *classes_array_ptr = use_classes ? reinterpret_cast<PyArrayObject *>(classes_array) : nullptr;
    // Check that the input array respect the dims
	if ((int)PyArray_NDIM(points_array_ptr) != 2 || (int)PyArray_DIM(points_array_ptr, 1) != 3)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(classes_array);
		Py_XDECREF(features_array);
	...
	if (use_feature && ((int)PyArray_NDIM(features_array_ptr) != 2))
	{
		Py_XDECREF(points_array);
		Py_XDECREF(classes_array);
		Py_XDECREF(features_array);
	...
	if (use_classes && (int)PyArray_NDIM(classes_array_ptr) > 2)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(classes_array);
		Py_XDECREF(features_array);
	...
	// Number of points
	int N = (int)PyArray_DIM(points_array_ptr, 0);

	// Dimension of the features
	int fdim = 0;
	if (use_feature)
	    fdim = (int)PyArray_DIM(features_array_ptr, 1);

	//Dimension of labels
	int ldim = 1;
	if (use_classes && (int)PyArray_NDIM(classes_array_ptr) == 2)
	    ldim = (int)PyArray_DIM(classes_array_ptr, 1);

	// Check that the input array respect the number of points
	if (use_feature && (int)PyArray_DIM(features_array_ptr, 0) != N)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(classes_array);
		Py_XDECREF(features_array);
	...
	if (use_classes && (int)PyArray_DIM(classes_array_ptr, 0) != N)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(classes_array);
		Py_XDECREF(features_array);
	...
	original_points = vector<PointXYZ>((PointXYZ*)PyArray_DATA(points_array_ptr), (PointXYZ*)PyArray_DATA(points_array_ptr) + N);
	if (use_feature)
		original_features = vector<float>((float*)PyArray_DATA(features_array_ptr), (float*)PyArray_DATA(features_array_ptr) + N*fdim);
	if (use_classes)
		original_classes = vector<int>((int*)PyArray_DATA(classes_array_ptr), (int*)PyArray_DATA(classes_array_ptr) + N*ldim);
